...
console.log('=== ヘッダー確認 ===');
console.log(
  `  企業ID 列: ${companyIdIdx >= 0 ? `index ${companyIdIdx} (${String.fromCharCode(65 + companyIdIdx)}列)` : 'なし ⚠'}`
);
console.log(
  `  Place ID 列: ${placeIdIdx >= 0 ? `index ${placeIdIdx} (${String.fromCharCode(65 + placeIdIdx)}列)` : 'なし ⚠'}`
);
...
check(
  ...
  invalidIds.length === 0,
  invalidIds.length
    ? `非準拠: ${invalidIds.slice(0, 3).join(', ')}`
    : `${ids.filter((i) => i).length} 件すべて OK`
);
...
for (const row of dataRows.slice(0, 5)) {
  ...
  console.log(
    `  ${cidOk ? '✔' : '✗'} ${name} | 企業ID="${cid}" | Place ID="${pid}" | Tel="${phone}"`
  );
}
